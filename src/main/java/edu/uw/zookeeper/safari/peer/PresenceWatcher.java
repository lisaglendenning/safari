...
public class PresenceWatcher extends PromiseTask<AbsoluteZNodePath, Identifier> implements WatchMatchListener, Runnable, Callable<Optional<Identifier>> {
    ...
    public static PresenceWatcher create(Identifier peer, ControlMaterializerService control, Promise<Identifier> promise) {
        return new PresenceWatcher(peer, control, promise);
    }
    ...
    protected PresenceWatcher(Identifier peer, ControlMaterializerService control, Promise<Identifier> promise) {
    ...
    public synchronized Optional<Identifier> call() throws Exception {
        if (!request.isPresent()) {
            ...
            control.submit(Operations.Requests.sync().setPath(task()).build());
            request = Optional.of(control.submit(Operations.Requests.exists().setPath(task()).setWatch(true).build()));
